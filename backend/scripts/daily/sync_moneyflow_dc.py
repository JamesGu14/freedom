...
from app.data.tushare_client import fetch_moneyflow_dc  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sync DC money flow data from TuShare moneyflow_dc into parquet partitions."
    )
...
def save_moneyflow_dc(df: pd.DataFrame) -> int:
    if df is None or df.empty:
        return 0

    features_base = settings.data_dir / "features" / "moneyflow_dc"
...
def flush_year_buffer(year: str, frames: list[pd.DataFrame]) -> tuple[int, int]:
    ...
    saved = save_moneyflow_dc(year_df)
    logger.info(
...
def main() -> None:
    ...
    logger.info(
        "sync_moneyflow_dc start: start=%s end=%s days=%s sleep=%.2f",
        start_date,
        end_date,
        len(date_list),
        args.sleep,
    ...
    progress = tqdm(date_list, total=len(date_list), desc="sync_moneyflow_dc", unit="day", dynamic_ncols=True)
    for idx, trade_date in enumerate(progress, start=1):
        ...
        try:
            ...
            if raw_df is None or raw_df.empty:
                # Dates before 2023-09-11 have no data — this is expected
    ...
    for d in synced_dates:
        mark_sync_done(d, "sync_moneyflow_dc")
    logger.info(
        "sync_moneyflow_dc done: synced_days=%s skipped_non_trading=%s api_rows=%s buffered_rows=%s saved_rows=%s",
        synced_days,
        skipped_non_trading,
        total_api_rows,
        total_buffered,
