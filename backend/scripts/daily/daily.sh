...
if [[ "${TARGET_WEEKDAY}" == "5" ]]; then
  run_task "同步申万行业成分(每周五)" "python backend/scripts/daily/sync_shenwan_members.py --incremental"
else
  echo "[INFO] $(date '+%F %T') 非周五，跳过申万行业成分同步" | tee -a "${LOG_FILE}"
fi
...
run_task "同步指数数据(中信/大盘/申万因子)" "python backend/scripts/daily/sync_zhishu_data.py --start-date ${START_DATE} --end-date ${END_DATE} --modules all --skip-members"

# 7) Compact fragmented parquet files (weekly to reduce daily runtime)
if [[ "${TARGET_WEEKDAY}" == "5" ]]; then
  run_task "压缩Parquet文件(每周五)" "python backend/scripts/daily/compact_parquet.py --dataset all"
else
  echo "[INFO] $(date '+%F %T') 非周五，跳过Parquet压缩" | tee -a "${LOG_FILE}"
fi
