...
CHECK_DATE="${END_DATE//-/}"
IS_OPEN="$(
CHECK_DATE="${CHECK_DATE}" PYTHONPATH="${ROOT_DIR}" python - <<'PY'
import sys
...
run_step_task "1" "拉取每日个股日线" "python /app/scripts/daily/pull_daily_history.py --start-date ${START_DATE} --end-date ${END_DATE}"

# 2) Sync technical factors for the date range
run_step_task "2" "同步每日技术因子" "python /app/scripts/daily/sync_stk_factor_pro.py --start-date ${START_DATE} --end-date ${END_DATE}"

# 3) Calculate daily signals for the date range
skip_step "3" "计算每日信号" "暂时注释禁用"
# run_step_task "3" "计算每日信号" "python -m scripts.daily.calculate_signal --start-date ${START_DATE} --end-date ${END_DATE}"

# 4) Sync Shenwan industry members (incremental)
TARGET_WEEKDAY="$(
END_DATE="${END_DATE}" PYTHONPATH="${ROOT_DIR}" python - <<'PY'
import datetime as dt
import os
...
if [[ "${TARGET_WEEKDAY}" == "5" ]]; then
  run_step_task "4" "同步申万行业成分(每周五)" "python /app/scripts/daily/sync_shenwan_members.py --incremental --sync-date ${END_DATE}"
else
  skip_step "4" "同步申万行业成分(每周五)" "非周五"
fi

# 5) Sync Shenwan daily index for the date range
run_step_task "5" "同步申万行业日线" "python /app/scripts/daily/sync_shenwan_daily.py --start-date ${START_DATE} --end-date ${END_DATE}"

# 6) Sync index-related data (CITIC daily, market dailybasic, index factors)
run_step_task "6" "同步指数数据(中信/大盘/申万因子)" "python /app/scripts/daily/sync_zhishu_data.py --start-date ${START_DATE} --end-date ${END_DATE} --modules all --skip-members"

# 7) Compact fragmented parquet files (weekly to reduce daily runtime)
if [[ "${TARGET_WEEKDAY}" == "5" ]]; then
  run_step_task "7" "压缩Parquet文件(每周五)" "python /app/scripts/daily/compact_parquet.py --dataset all --sync-date ${END_DATE}"
else
