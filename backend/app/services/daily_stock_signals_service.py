...
def _truncate_group_stocks(group: dict[str, Any], top_n: int) -> dict[str, Any]:
    doc = dict(group)
    stocks = list(doc.get("stocks") or [])[:top_n]
    doc["stocks"] = [
        {
            ...
            "patterns": s.get("patterns", [])[:10] if isinstance(s.get("patterns"), list) else [],
        }
        for s in stocks
    ]
    return doc
...
def get_stock_recent_signals(*, ts_code: str, limit_days: int = 30) -> list[dict[str, Any]]:
    ...
    def _forward_return(trade_date: str, n_days: int) -> float | None:
        ...
        if target_idx >= len(ordered_dates):
            return None
        base = price_map.get(trade_date)
        target = price_map.get(ordered_dates[target_idx])
...
def get_stock_pattern_details(*, ts_code: str, trade_date: str) -> dict[str, Any] | None:
    from app.data.mongo import get_collection
    from app.signals.patterns.config import get_pattern_weight, get_pattern_category_label
