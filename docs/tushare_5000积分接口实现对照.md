...
| 股票数据 / 财务数据       | 业绩预告       | `forecast`             | `forecast`                               | DuckDB  | `backend/scripts/daily/sync_financial_reports.py --dataset forecast`                            | 未纳入当前财务审计；新接入接口 |
| 股票数据 / 财务数据       | 业绩快报       | `express`              | `express`                                | DuckDB  | `backend/scripts/daily/sync_financial_reports.py --dataset express`                             | 未纳入当前财务审计；新接入接口 |
| 股票数据 / 财务数据       | 财务审计意见     | `fina_audit`           | `fina_audit`                             | DuckDB  | `backend/scripts/daily/sync_fina_audit.py`                                                      | 未纳入当前财务审计；新接入接口；ts_code 必填，需遍历股票 |
| 股票数据 / 财务数据       | 主营业务构成     | `fina_mainbz`          | `fina_mainbz`                            | DuckDB  | `backend/scripts/daily/sync_fina_mainbz.py`                                                     | 未纳入当前财务审计；新接入接口；ts_code 必填，按报告期查询，每季度手动运行 |
| 股票数据 / 财务数据       | 财报披露日期表    | `disclosure_date`      | `disclosure_date`                        | DuckDB  | `backend/scripts/daily/sync_disclosure_date.py`                                                 | 未纳入当前财务审计；新接入接口；按报告期查全市场 |
...
- 如果只看这份 `5000 积分可用接口` 清单，当前仓库已经接入了 `34 / 179` 个接口，其中 `33` 个已经形成本地数据集，`1` 个只做了按需调用。
